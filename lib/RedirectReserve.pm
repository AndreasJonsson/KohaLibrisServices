package RedirectReserve;

@ISA         = qw(Exporter);
@EXPORT      = qw(redirect_reserve_app);
@EXPORT_OK   = qw();
...
sub redirect_reserve_app {
	...
	if ($@) {
            warn "RedirectReserve: $@\n";
            $loc = '/cgi-bin/koha/errors/500.pl';
            $code = '303';
        } elsif (defined($row) && defined($row->{biblionumber})) {
            $loc = '/cgi-bin/koha/opac-reserve.pl?biblionumber=' . uri_escape($row->{biblionumber});
            $code = '301';
	} else {
            $code = '303';
